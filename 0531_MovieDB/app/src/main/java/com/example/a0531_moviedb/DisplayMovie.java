package com.example.a0531_moviedb;
...
public class DisplayMovie extends AppCompatActivity {
    ...
    public void insert(View view) {
        ...
        if (extras != null) {
            int Value = extras.getInt("id");
            if (Value > 0) {
                if (mydb.updateMovie(id, name.getText().toString(),
                    ...
                    Intent intent = new Intent(getApplicationContext(), com.example.a0531_moviedb.MainActivity.class);
                    startActivity(intent);
                } else {
                    Toast.makeText(getApplicationContext(), "수정되지 않았음", Toast.LENGTH_SHORT).show();
                }
            } else {
                if (mydb.insertMovie(name.getText().toString(),
                        director.getText().toString(),
                        year.getText().toString(),
                        nation.getText().toString(),
                        rating.getText().toString())) {
